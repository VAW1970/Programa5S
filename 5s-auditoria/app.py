...
SENSES_INFO = {
    "Seiri (Utilização)": {"icon": "🔍", "color": "#e74c3c", "desc": "Separar o necessário do desnecessário"},
    "Seiton (Organização)": {"icon": "📦", "color": "#f39c12", "desc": "Organizar e ter tudo no lugar"},
    "Seiso (Limpeza)": {"icon": "✨", "color": "#27ae60", "desc": "Manter o local limpo"},
    "Seiketsu (Padronização)": {"icon": "📋", "color": "#3498db", "desc": "Padronizar processos"},
    "Shitsuke (Disciplina)": {"icon": "🎯", "color": "#9b59b6", "desc": "Manter os hábitos"},
}
...
def salvar_auditoria():
    ...
    for senso, items in SENSOS_ITEMS.items():
        for idx, item in enumerate(items):
            ...
            if nota is not None:
                db.execute(
                    "INSERT INTO itens_auditoria (auditoria_id, senso, item, nota) VALUES (?, ?, ?, ?)",
